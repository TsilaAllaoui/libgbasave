...
namespace gbasave
{
    ...
    {
        std::vector<std::uint8_t> m = input;
        const std::uint64_t bits = static_cast<std::uint64_t>(m.size()) * 8;
        m.push_back(0x80);
        while ((m.size() % 64) != 56)
            m.push_back(0);
        for (int i = 7; i >= 0; --i)
            m.push_back(static_cast<std::uint8_t>(bits >> (i * 8)));
        ...
        for (std::size_t off = 0; off < m.size(); off += 64)
        {
            std::uint32_t w[64]{};
            for (int i = 0; i < 16; ++i)
                w[i] = (std::uint32_t(m[off + i * 4]) << 24) | (std::uint32_t(m[off + i * 4 + 1]) << 16) | (std::uint32_t(m[off + i * 4 + 2]) << 8) | m[off + i * 4 + 3];
            for (int i = 16; i < 64; ++i)
            {
