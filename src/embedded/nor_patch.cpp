    ...
    for (uint32_t i = 0u; i < savePlan.op_count; ++i) {
        ...
        if (patch.route == GBASAVE_NOR_PATCH_RWW_EEPROM && op.kind == SFW_OP_EEPROM_WRITE) {
            ...
            const uint32_t target = kGbaRomBase + patch.payload_offset +
                static_cast<uint32_t>(gbasave::generated_direct_rww_assets::kEepromEepromWriteAuto) + 1u;
    ...
    GbasaveNorPatchPlan *out)
{
    ...
    if (layout->route == GBASAVE_NOR_SAVE_LAYOUT_RWW_SRAM && romPlan->save_type == SFW_SAVE_SRAM) {
        asset = kSramRwwPayload; assetBytes = static_cast<uint32_t>(kSramRwwPayloadSize);
        ...
        asset = kEepromDualRwwPayload; assetBytes = static_cast<uint32_t>(kEepromDualRwwPayloadSize);
        ...
        asset = kFlashCompactRwwPayload; assetBytes = static_cast<uint32_t>(kFlashCompactRwwPayloadSize);
